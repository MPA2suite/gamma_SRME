...
from gamma_SRME.utils import aseatoms2str, log_message, log_symmetry
...
def two_stage_relax(
    ...
    if not allow_tilt:
        ...
        # tilt_mask = [False, False, False, False, False, False]
    ...
    log_message(
        f"Initial Stress {atoms.get_stress()*convert_ase_to_bar} bar", output=log
    )
    ...
    total_filter = Filter(
        atoms, mask=tilt_mask, constant_volume=constant_volume, **_filter_kwargs
    )
    dyn_stage1 = Optimizer(total_filter, **_optim_kwargs, logfile=ase_logfile)
    dyn_stage1.run(fmax=fmax_stage1, steps=steps_stage1)

    log_message(
        f"After keeping symmetry stage 1 relax, energy {atoms.get_potential_energy()} ev",
        output=log,
    )
    log_message(
        f"After keeping symmetry stage 1 relax, stress {atoms.get_stress()*convert_ase_to_bar} bar",
        output=log,
    )
    ...
    if sym_stage1["number"] != sym_init["number"]:
        warnings.warn(
            f"Symmetry is not kept during FixSymmetry relaxation of material {mat_name} in folder {os.getcwd()}"
        )
        log_message(
            f"Symmetry is not kept during FixSymmetry relaxation of material {mat_name} in folder {os.getcwd()}",
            output=log,
    ...
    log_message(
        "Stage 2 Stress", atoms.get_stress() * convert_ase_to_bar, " bar", output=log
    )
    ...
    log_message(f"Final cell parameters\n {return_atoms.get_cell()[:]}", output=log)
...
def simple_relax(
    ...
    **kwargs,
    ...
    filter = Filter(atoms, constant_volume=constant_volume, mask=tilt_mask, **kwargs)
    dyn = Optimizer(filter, logfile=ase_logfile, **_optim_kwargs)
